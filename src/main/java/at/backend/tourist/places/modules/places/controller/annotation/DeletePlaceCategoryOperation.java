...
@Operation(
    summary = "Delete a place category by ID",
    description = "Permanently remove a place category from the system using its unique identifier. **Requires ADMIN role**. This action cannot be undone.",
    security = @SecurityRequirement(name = "bearerAuth")
)
@ApiResponses({
    @ApiResponse(
        responseCode = "204",
        description = "Place category deleted successfully",
        content = @Content(
            mediaType = "application/json",
            schema = @Schema(implementation = ResponseWrapper.class),
            examples = @io.swagger.v3.oas.annotations.media.ExampleObject(
                name = "Success Response",
                value = ApiResponseExamples.PLACE_CATEGORY_DELETED
            )
        )
    ),
    @ApiResponse(
...
public @interface DeletePlaceCategoryOperation {
}
