...
@Operation(
    summary = "Get tourist places by country",
    description = "Retrieve a comprehensive list of all tourist places located in a specific country. Returns multiple tourist places with complete details for each one."
)
@ApiResponses({
    @ApiResponse(
        responseCode = "200",
    ...
    @ApiResponse(
        responseCode = "404",
        description = "No tourist places found for the specified country",
        content = @Content(
            mediaType = "application/json",
            schema = @Schema(implementation = ResponseWrapper.class),
            examples = @io.swagger.v3.oas.annotations.media.ExampleObject(
...
public @interface GetTouristPlacesByCountryOperation {
}
