...
@Operation(
    ...
    description = "Add a new tourist place to the system with complete details including name, description, location, rating, opening hours, price range, country, and category. **Requires ADMIN role**.",
    security = @SecurityRequirement(name = "bearerAuth")
)
@ApiResponses({
    @ApiResponse(
        responseCode = "201",
        description = "Tourist place created successfully",
        content = @Content(
            mediaType = "application/json",
            schema = @Schema(implementation = ResponseWrapper.class),
            examples = @io.swagger.v3.oas.annotations.media.ExampleObject(
                name = "Success Response",
                value = ApiResponseExamples.TOURIST_PLACE_CREATED
            )
        )
    ),
    @ApiResponse(
...
public @interface CreateTouristPlaceOperation {
}
