...
@Operation(
    ...
    description = "Add a new place category to the system with a name and description. **Requires ADMIN role**.",
    security = @SecurityRequirement(name = "bearerAuth")
)
@ApiResponses({
    @ApiResponse(
        responseCode = "201",
        description = "Place category created successfully",
        content = @Content(
            mediaType = "application/json",
            schema = @Schema(implementation = ResponseWrapper.class),
            examples = @io.swagger.v3.oas.annotations.media.ExampleObject(
                name = "Success Response",
                value = ApiResponseExamples.PLACE_CATEGORY_CREATED
            )
        )
    ),
    @ApiResponse(
...
public @interface CreatePlaceCategoryOperation {
}
