...
@Operation(
    ...
    description = "Add a new activity to the system with complete details. **Requires ADMIN role**. The activity will be associated with a specific tourist place and includes pricing and duration information.",
    security = @SecurityRequirement(name = "bearerAuth")
)
@ApiResponses(value = {
    @ApiResponse(
        responseCode = "201",
        description = ApiConstants.ACTIVITY_CREATED,
        content = @Content(
            mediaType = "application/json",
            schema = @Schema(implementation = ResponseWrapper.class),
            examples = @io.swagger.v3.oas.annotations.media.ExampleObject(
                name = "Success Response",
                value = ApiResponseExamples.ACTIVITY_CREATED
            )
        )
    ),
    @ApiResponse(
        responseCode = "400",
        description = ApiConstants.INVALID_INPUT_DATA,
        content = @Content(
            mediaType = "application/json",
            schema = @Schema(implementation = ResponseWrapper.class),
            examples = @io.swagger.v3.oas.annotations.media.ExampleObject(
...
public @interface CreateActivityOperation {
}
