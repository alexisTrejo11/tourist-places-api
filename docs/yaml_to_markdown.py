...
# ---------------------------------------------------------------------------
# Schema validation (docs/source/*.md ≈ docs/source/schema.ts)
# ---------------------------------------------------------------------------

SOURCE_SCHEMA: dict[str, dict[str, Any]] = {
    "ProjectMetadata.md": {
        "required": [
            "projectId", "name", "description", "language", "framework",
            "version", "category", "status", "featured", "repositoryUrl",
            "liveDemoUrl", "techStack", "createdAt", "updatedAt",
        ],
        "list_keys": ["techStack"],
    },
    "ProjectOverview.md": {
        "required": [
            "problemStatement", "solution", "keyMetrics", "links",
            "mediaGallery", "mediaItems", "metrics",
        ],
        "nested": {
            "problemStatement": ["problemTitle", "problemDescription", "problemList"],
            "solution": ["solutionTitle", "solutionList"],
            "keyMetrics": ["metricsTitle", "metricsList"],
            "links": ["github", "demo", "documentation", "dockerHub"],
            "mediaGallery": ["title", "items"],
        },
    },
    "APISchema.md": {
        "required": ["type", "httpEndpoints"],
        "endpoint_keys": [
            "id", "method", "urlPath", "summary", "description", "tags",
            "authenticated", "rateLimit", "responses",
        ],
    },
    "ProjectFeature.md": {
        "required": ["features"],
        "feature_keys": [
            "id", "title", "description", "icon", "category", "status", "highlights",
        ],
    },
    "ProjectArchitecture.md": {
        "required": [
            "layers", "designPatterns", "scalabilityStrategies", "securityStrategies",
            "cacheStrategies", "architectureFeatures", "architectureDiagram",
            "dataFlow", "techDecisions",
        ],
    },
    "ProjectInfrastructure.md": {
        "required": ["metrics", "cloudServices", "deploymentLayers", "dockerFiles"],
    },
    "ProjectCodeShowCase.md": {
        "required": ["codeExamples"],
        "example_keys": ["id", "title", "description", "category", "files"],
        "file_keys": ["name", "path", "language", "content"],
    },
}
...
def validate_source_file(name: str, data: dict[str, Any]) -> list[str]:
    """Return list of validation errors; empty means OK for schema.ts contract."""
    errors: list[str] = []
    spec = SOURCE_SCHEMA.get(name)
    if not spec:
        return errors

    for key in spec.get("required", []):
        if key not in data or _is_blank(data[key]):
            errors.append(f"{name}: missing or empty required key '{key}'")

    for key in spec.get("list_keys", []):
        val = data.get(key)
        if isinstance(val, list) and len(val) == 0:
            errors.append(f"{name}: list '{key}' must not be empty")

    for parent, child_keys in spec.get("nested", {}).items():
        block = data.get(parent)
        if not isinstance(block, dict):
            continue
        for ck in child_keys:
            if ck not in block or _is_blank(block[ck]):
                errors.append(f"{name}: {parent}.{ck} missing or empty")

    for ep in data.get("httpEndpoints") or []:
        if not isinstance(ep, dict):
            continue
        for ek in spec.get("endpoint_keys", []):
            if ek not in ep or _is_blank(ep[ek]):
                errors.append(f"{name}: endpoint '{ep.get('id', '?')}' missing '{ek}'")

    for feat in data.get("features") or []:
        if not isinstance(feat, dict):
            continue
        for fk in spec.get("feature_keys", []):
            if fk not in feat or _is_blank(feat[fk]):
                errors.append(f"{name}: feature '{feat.get('id', '?')}' missing '{fk}'")

    for ex in data.get("codeExamples") or []:
        if not isinstance(ex, dict):
            continue
        for ek in spec.get("example_keys", []):
            if ek not in ex or _is_blank(ex[ek]):
                errors.append(f"{name}: codeExample '{ex.get('id', '?')}' missing '{ek}'")
        for f in ex.get("files") or []:
            if not isinstance(f, dict):
                continue
            for fk in spec.get("file_keys", []):
                if fk not in f or _is_blank(f[fk]):
                    errors.append(
                        f"{name}: file in '{ex.get('id', '?')}' missing '{fk}'"
                    )

    return errors


def validate_all_sources(input_dir: Path) -> list[str]:
    all_errors: list[str] = []
    for src in sorted(input_dir.glob("*.md")):
        text = src.read_text(encoding="utf-8")
        data, _ = split_frontmatter(text)
        all_errors.extend(validate_source_file(src.name, data))
    return all_errors
...
def write_index(output_dir: Path, generated: list[str]) -> None:
    lines = [
        "# Tourist Places API — Generated documentation",
        "",
        "Auto-generated from `docs/source/` YAML frontmatter (contract: `docs/source/schema.ts`).",
        "Regenerate with:",
        "",
        "```bash",
        "python docs/yaml_to_markdown.py",
...
def main() -> int:
    ...
    parser.add_argument(
        "--validate",
        action="store_true",
        help="Validate docs/source YAML against schema.ts field contract (exit 1 on failure)",
    )
    ...
    if args.validate:
        errors = validate_all_sources(input_dir)
        if errors:
            print("Schema validation failed (docs/source must match schema.ts):", file=sys.stderr)
            for err in errors:
                print(f"  - {err}", file=sys.stderr)
            return 1
        print("Schema validation passed — docs/source matches schema.ts contract.")
